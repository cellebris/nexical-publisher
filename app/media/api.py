...
from app.utils.file import get_file_content, get_url_content
...
#
def validate_media(self, validated_data):
...
@extend_schema_field(OpenApiTypes.STR)
def field_url_content(self, instance):
    return get_url_content(instance.url)


#
# Update Handler
#
def create_media_collection_event(self, instance):
    if not self.context["request"].user.check_member("engine"):
        instance.create_event()


def create_media_event(self, instance):
    if not self.context["request"].user.check_member("engine"):
...
serializers.generate(
    models.TeamMedia, class_prefix="MediaCollection", fields=["id", "file", "description", "created", "updated"]
)
serializers.generate(
    models.TeamMediaBookmark, class_prefix="MediaCollection", fields=["id", "url", "description", "created", "updated"]
)
...
views.generate(
    ...
    get_queryset=get_media_queryset,
    ordering_fields=["id"],
...
views.generate(
    ...
    relation_fields=["collection"],
    object_method_fields={"content": field_url_content},
...
views.generate(
    ...
    reverse_prefix="MediaCollection",
    view_reverse_fields=["files", "bookmarks"],
    handler=create_media_collection_event,
)
